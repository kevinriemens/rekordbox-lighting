...
**Note:** Track→bank assignment work now lives in `.opencode/refined/TRACKLIGHT-epic.md`. New ideas in that area belong there rather than as loose backlog items.
...
- [ ] **S2.1 — Role-based YAML macro recipes** *(added 2026-08-25)* — **M**
   *(TRACKLIGHT Stage 2 story; see `.opencode/refined/TRACKLIGHT-epic.md`)*
...
**Note:** M4 — Phrase/pattern rebalance has been superseded by and merged into the TRACKLIGHT epic (Stage 1). It described the same mechanism (`content.macro_pattern_id` rewriting) with a load-balancing goal instead of a taste-driven one. The work is now the `S1.2` story in `.opencode/refined/TRACKLIGHT-epic.md`.
