---
epic: "TRACKLIGHT"
title: "Stage 2.3 — FullArcAI venue (breaking the bar mirror)"
estimate: L
status: ready
created: 2026-08-26
depends_on: ["RIG-calibration-session"]
...
The earlier pitch claimed `FullArcAI` would unlock "a continuous left-to-right sweep across all 18 bar cells" — meaning the 18 cells alone, sharing one horizontal axis. **That specific framing is physically impossible and must not be built that way.** Both L1015 bars are mounted VERTICALLY, on the inside face of the two 150cm arch legs. Their cells run UP THE POLE. There is no shared horizontal axis between the two bars' cells — each bar's 9 cells can only move up and down its own leg. A left-to-right sweep built from cell motion alone was never available.

A previous documentation pass over-corrected this and concluded the DJ's "left-to-right sweep" itself was impossible and replaced it outright with vertical per-leg motion. Both are correct, and the DJ confirms he wants to build the sweep: it is not a horizontal pan across the room, it is a **travelling path around the rig's perimeter** — starting low on the left, climbing vertically through bar 1's cells, handing off to the left 45° LM70S head, crossing the top through the two horizontal LM70S heads, handing off to the right 45° LM70S head, and descending through bar 2's cells. It reads left-to-right as a whole gesture while being built almost entirely out of vertical, per-leg cell motion and per-fixture handoffs — see `physical-rig-profile` for the named gesture and its fixture ordering. The vertical per-leg motion this story's non-mirrored slot assignment enables is not a replacement for the sweep; it is the mechanism the sweep is built from.

What survives from the earlier pitch is the *arithmetic*, which is still valid and still the real reason to do this. What needed replacing was never the sweep itself — only the assumption that it could be built from the 18 cells' shared horizontal motion instead of from per-leg vertical motion chained with the heads.
...
- **Horizontal bar sweeps** via the tilt blocks, which tilt each whole bar as a unit and are already separate fixtures (slots 111, 112)
- **The four heads moving in 3D**, on the two diagonals and the top, independent of the bars
- **The perimeter sweep** — the DJ's "left-to-right sweep": a travelling path from the bottom of the left leg, up through bar 1's cells, across the two 45° heads and the two horizontal heads, down through bar 2's cells. This is the gesture that most directly needs the non-mirrored assignment — it depends on bar 1 and bar 2 running independent vertical motion at different points in the phrase, which the hard-mirrored slot sequence cannot express today
...
* [ ] **Scenario 3: 25-slot invariant maintained**
    ...
    * When the user inspects the venue's fixture rows
    * Then exactly 25 macro fixture slots are referenced across all 27 fixtures, and the invariant holds: every macro has exactly 25 `macro_data` rows (one per slot)
...
* [ ] **Scenario 4: Dry run shows complete assignment table without writing**
    ...
    * Then the complete cell-to-slot assignment table is displayed for review, showing which fixtures map to which slots and which slots are shared; no changes are written to the database
...
* [ ] **Scenario 6: ExecVenueId and LastSelectedVenue untouched**
    ...
    * When the new venue is created
    * Then `ExecVenueId` and `LastSelectedVenue` remain unchanged in `lighting_property`
...
* [ ] **Scenario 9: User can confirm on the real rig**
    ...
    * Then the two legs perform different actions (e.g., one rises while the other falls), confirming the non-mirrored assignment is in effect

* [ ] **Edge Case: Unpatched 4th par remains unreachable**
    * Given the 4th par has no DMX address in the current patch
    * When the new venue is created
...
* **Database**: Persist a new venue row in `user.db3` with 27 fixture rows, each with a deliberately chosen `macro_fixture_id` slot assignment that breaks the bar mirror. The assignment must be expressed as data (declarative table or spec), not buried in imperative code, so it can be diffed, reviewed, and revised.
...
- **Hardware**: 4x LM70S moving head (14 channels each, at DMX 1/15/29/43), 2x L1015 moving beam bar (43 channels each, at DMX 57 and 100) — the whole bar tilts as one unit and has 9 independently addressable pixel cells, with no independent pan. 3x LPC008S pars patched at DMX 143/150/157; the 4th par is unpatched and unreachable.

- **Truss**: 5-segment arch — 150cm vertical left leg, 100cm diagonal at 45° up-right, 100cm horizontal top, 100cm diagonal at 45° down-right, 150cm vertical right leg. Bounding box roughly 241cm × 221cm. One LM70S on each diagonal, two along the horizontal top. Both bars mounted VERTICALLY on the two legs.
...
- No horizontal sweep built from the 18 cells sharing one horizontal axis — the two bars have no such shared axis, only independent vertical motion per leg. (The DJ's "left-to-right sweep" is a real, separate gesture spanning bars *and* heads together — see "The Correction" above and `physical-rig-profile`. This story enables it by breaking the bar mirror; it does not author the sweep macro itself.)
...
- `.opencode/skills/physical-rig-profile/SKILL.md`: record the new venue and its assignment once built, and make sure the cells-have-no-shared-horizontal-axis correction and the perimeter-sweep gesture are both stated there
...
| `.opencode/skills/physical-rig-profile/SKILL.md` | New venue and assignment documented; cells-have-no-shared-horizontal-axis correction and perimeter-sweep gesture both stated |

## 8. Dependencies & Ordering

- **Depends on** `RIG-calibration-session`, specifically the items confirming physical truss geometry and fixture positions. Building a new fixture-to-slot assignment on an unverified physical mapping risks designing the wrong show.

- **Is orthogonal to Stage 1 and Stage 2's bank takeover (S2.2).** It concerns venue and fixture layout in `user.db3`, not `content.macro_pattern_id` or `macro_assign`. These can be built in parallel.

- **Relates to** S2.1 (role-based YAML macro recipes) and S2.2 (bank takeover), which together will provide the custom macro content that the new venue's non-mirrored assignment will exploit.
...
- `physical-rig-profile` — rig layout, fixture positions, bar orientation, DMX addressing, current slot occupancy
